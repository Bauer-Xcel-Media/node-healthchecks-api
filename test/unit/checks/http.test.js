...
const makeMethodMock = mockBody => {
    const handler = jest.fn(url => {
        ...
        promise.set = jest.fn((name, value) => promise);
        promise.timeout = jest.fn(value => promise);
        return promise;
    });
    return handler;
};
...
const testStatusChange = async (body, expected) => {
    const { Testee, mockHealth, superagent } = makeMocks(body);
    const options = {
...
it ('should instantiate the Http check class properly', async () => {
    const { Testee, mockMethod, superagent } = makeMocks(['OK']);
    const options = {
        url: 'http://myhost:3000',
    }
    const testeeInstance = new Testee(options);
...
it ('should set the status to OK when the http response includes OK', () => testStatusChange([ constants.OK ], constants.OK));

it ('should set the status to WARN when the http response includes WARN', () => testStatusChange([ constants.WARN ], constants.WARN));

it ('should set the status to CRIT when the http response includes CRIT', () => testStatusChange([ constants.CRIT ], constants.CRIT));

it ('should set the status to OK for non-traversable dependency', () => testStatusChange('non-traversable', constants.OK));
