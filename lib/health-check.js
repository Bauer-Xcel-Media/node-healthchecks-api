...
module.exports = async (server, options = {
    ...
    let _adapter = adapter instanceof Function ? adapter : adapters[adapter];
    if (!_adapter) {
    ...
    if (!service.config) {
        ...
        if (service.config && service.config.checks) {
            service.checks = await Promise.all(service.config.checks.map(async function (checkConfig) {
                ...
                _check.on('statusChanged', function (check) {
                    if (this.checks) {
                        this.checks.sort(checksSorter);
                    }
                }.bind(this));
                return await _check.start();
            }.bind(service)));
        }
    }
...
module.exports.addChecks = async (checks) => {
    if (checks instanceof Check) {
        CHECKS[checks.type] = checks;
    } else if (typeof checks === 'string') {
        return await addCheck(await utils.linkAndRequire(checks));
